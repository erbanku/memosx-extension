...
$("textarea[name=text]").blur(function () {
    chrome.storage.sync.set({
        ...
        open_content: $("textarea[name=text]").val(),
    });
});
                    ...
                    if (data.id) {
                        ...
                        chrome.storage.sync.set(
                            ...
                            function () {
                                $.message({
                                    message:
                                        chrome.i18n.getMessage("picSuccess"),
                                });
                            }
                        );
                    } else {
                        ...
                        chrome.storage.sync.set(
                            ...
                            function () {
                                $.message({
                                    message:
                                        chrome.i18n.getMessage("picFailed"),
                                });
                            }
                        );
                    }
...
$(document).on("click", ".item-lock", function () {
    ...
    chrome.storage.sync.set({ memo_lock: _this });
});
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        for (var i = 0; i < searchData.length; i++) {
                            searchDom +=
                                '<div class="random-item"><div class="random-time"><span id="random-link" data-id="' +
                                searchData[i].item.id +
                                '"><svg class="icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" width="32" height="32"><path d="M864 640a32 32 0 0 1 64 0v224.096A63.936 63.936 0 0 1 864.096 928H159.904A63.936 63.936 0 0 1 96 864.096V159.904C96 124.608 124.64 96 159.904 96H384a32 32 0 0 1 0 64H192.064A31.904 31.904 0 0 0 160 192.064v639.872A31.904 31.904 0 0 0 192.064 864h639.872A31.904 31.904 0 0 0 864 831.936V640zm-485.184 52.48a31.84 31.84 0 0 1-45.12-.128 31.808 31.808 0 0 1-.128-45.12L815.04 166.048l-176.128.736a31.392 31.392 0 0 1-31.584-31.744 32.32 32.32 0 0 1 31.84-32l255.232-1.056a31.36 31.36 0 0 1 31.584 31.584L924.928 388.8a32.32 32.32 0 0 1-32 31.84 31.392 31.392 0 0 1-31.712-31.584l.736-179.392L378.816 692.48z" fill="#666" data-spm-anchor-id="a313x.7781069.0.i12" class="selected"/></svg></span><span id="random-delete" data-id="' +
                                searchData[i].item.id +
                                '"><svg class="icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" width="32" height="32"><path d="M224 322.6h576c16.6 0 30-13.4 30-30s-13.4-30-30-30H224c-16.6 0-30 13.4-30 30 0 16.5 13.5 30 30 30zm66.1-144.2h443.8c16.6 0 30-13.4 30-30s-13.4-30-30-30H290.1c-16.6 0-30 13.4-30 30s13.4 30 30 30zm339.5 435.5H394.4c-16.6 0-30 13.4-30 30s13.4 30 30 30h235.2c16.6 0 30-13.4 30-30s-13.4-30-30-30z" fill="#666"/><path d="M850.3 403.9H173.7c-33 0-60 27-60 60v360c0 33 27 60 60 60h676.6c33 0 60-27 60-60v-360c0-33-27-60-60-60zm-.1 419.8l-.1.1H173.9l-.1-.1V464l.1-.1h676.2l.1.1v359.7z" fill="#666"/></svg></span>' +
                                dayjs(
                                    new Date(searchData[i].item.createdTs) *
                                        1000
                                ).fromNow() +
                                '</div><div class="random-content">' +
                                searchData[i].item.content
                                    .replace(
                                        /!\[.*?\]\((.*?)\)/g,
                                        ' <img class="random-image" src="$1"/> '
                                    )
                                    .replace(
                                        /\[(.*?)\]\((.*?)\)/g,
                                ...
                                "</div>";
                            if (
                                searchData[i].item.resourceList &&
                                searchData[i].item.resourceList.length > 0
                            ) {
                                var resourceList =
                                    searchData[i].item.resourceList;
                                for (var j = 0; j < resourceList.length; j++) {
                                    var restype = resourceList[j].type.slice(
                                        0,
                                        5
                                    );
                                    var resexlink =
                                        resourceList[j].externalLink;
                                    var resLink = "",
                                        fileId = "";
                                    if (resexlink) {
                                        resLink = resexlink;
...
$("#random").click(function () {
    get_info(function (info) {
        if (info.status) {
            $("#randomlist").html("").hide();
            var nowTag = $("textarea[name=text]")
                .val()
                .match(/#([^\s#]+)/);
            if ($("#taglist").is(":visible") && nowTag[1]) {
                var tagUrl =
                    info.apiUrl +
                    "api/v1/memo?rowStatus=NORMAL&tag=" +
                    nowTag[1];
                $.ajax({
                    url: tagUrl,
                    type: "GET",
                ...
                var randomUrl0 =
                    info.apiUrl + "api/v1/memo?rowStatus=NORMAL&limit=1";
                $.ajax({
                    url: randomUrl0,
                    type: "GET",
                    contentType: "application/json;",
                    ...
                    success: function (data0) {
                        ...
                        $.ajax({
                            ...
                            success: function (data1) {
                                ...
                                var randomUrl2 =
                                    info.apiUrl +
                                    "api/v1/memo?rowStatus=NORMAL&limit=1&offset=" +
                                    randomNum;
                                $.ajax({
                                    url: randomUrl2,
                                    type: "GET",
        ...
        if (randomData.resourceList && randomData.resourceList.length > 0) {
            ...
            for (var j = 0; j < resourceList.length; j++) {
                ...
                if (resexlink) {
                    ...
                    fileId =
                        resourceList[j].publicId || resourceList[j].filename;
                    resLink =
                        info.apiUrl +
                        "o/r/" +
                        resourceList[j].id +
                ...
                if (restype == "image") {
                    randomDom +=
                        '<img class="random-image" src="' + resLink + '"/>';
                }
                if (restype !== "image") {
                    randomDom +=
                        '<a target="_blank" rel="noreferrer" href="' +
